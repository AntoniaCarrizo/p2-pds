...
export function Tema1() {
  ...
  tareas.length > 0 && currentQuestionIndex < tareas[0]?.preguntas.length
    ? getPreguntaData(tareas[0].preguntas[currentQuestionIndex])
    : null;
  ...
  const todasLasPreguntasRespondidas = currentQuestionIndex >= tareas[0]?.preguntas.length;
  ...
  return (
    <div className="min-h-screen bg-green-100 flex flex-col items-center justify-center">
      ...
      {currentQuestion ? (
        <>
          <div className="text-lg mb-2">🔅 Cantidad de preguntas: {tareas.length > 0 ? tareas[0].cantidad_preguntas : 'Cargando...'}</div>
...
{todasLasPreguntasRespondidas && todasLasPreguntasCorrectas ? (
        ...
        <div>
          ...
          <button className="bg-purple-200 rounded-full px-4 py-2 mt-4">
            <Link to="/tema1segundointento">Volver a intentar</Link>
          </button>
        </div>
      ) : null}
    </div>
